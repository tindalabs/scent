...
describe('detectRapidReregistration', () => {
  it('returns null below the 3-identity threshold', async () => {
    expect(await detectRapidReregistration(mockSql(0), 'p', 'abcd1234', 'cur')).toBeNull();
    expect(await detectRapidReregistration(mockSql(2), 'p', 'abcd1234', 'cur')).toBeNull();
  });

  it('flags at exactly 3 similar new identities', async () => {
    const flag = await detectRapidReregistration(mockSql(3), 'p', 'abcd1234', 'cur');
  ...
  it('confidence rises with more identities and caps at 0.90', async () => {
    const five = await detectRapidReregistration(mockSql(5), 'p', 'abcd1234', 'cur');
    const many = await detectRapidReregistration(mockSql(20), 'p', 'abcd1234', 'cur');
  ...
  it('reason names the count and the window', async () => {
    const flag = await detectRapidReregistration(mockSql(4), 'p', 'abcd1234', 'cur', 30);
