...
describe('detectCoordinatedBehavior', () => {
  it('returns null below the 2-identity threshold', async () => {
    expect(await detectCoordinatedBehavior(mockSql(0), 'p', 'i', 'abcd', null)).toBeNull();
    expect(await detectCoordinatedBehavior(mockSql(1), 'p', 'i', 'abcd', null)).toBeNull();
  });

  it('flags at exactly 2 sibling identities', async () => {
    const flag = await detectCoordinatedBehavior(mockSql(2), 'p', 'i', 'abcd', null);
  ...
  it('confidence rises with more siblings and caps at 0.88', async () => {
    const many = await detectCoordinatedBehavior(mockSql(20), 'p', 'i', 'abcd', 'cluster-1');
