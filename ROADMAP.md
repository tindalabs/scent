...
- **Admin RBAC + account management** (two-level RBAC + invite links + TOTP 2FA, staged across 3 PRs):
  ...
  - [ ] **PR2 — account-management UI**: owner-only Users page — invite via copy-paste link (opaque one-time token, no SMTP), list/role-change/soft-deactivate, membership grants, self-service password change.
  - [ ] **PR3 — 2FA**: TOTP (otplib) + hashed recovery codes, secret encrypted at rest, org-wide "require 2FA" toggle. (WebAuthn/passkeys = future.)
