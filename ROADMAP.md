...
- [x] **Per-project data viewing via the admin session** — the `/v1` read routes now accept an admin session + `X-Project-Id` (GET only) alongside the project key (`requireProjectRead`), and the Observatory has a project switcher that scopes every data page. The build-time `VITE_API_KEY` is gone. Ingest/resolve stay strictly key-gated. (Remaining: per-admin project ACLs — any admin can currently view any project, consistent with the existing admin API.)
